...
pub(super) fn build_launch_env(
    ...
    let mut env = if let Some((iso, paths)) = iso_setup {
        // Dispatch on the TARGET RUNTIME's spoof_home flag; harnesses inherit it
        // from their target runtime, and only Claude sets spoof_home=true.
